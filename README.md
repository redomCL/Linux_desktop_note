...
* nautilus启用rootmode和smb：root组件：sudo apt nautilus-admin，smb组件：sudo apt nautilus-share,部署smb：部署用户组：sudo usermod -aG sambashare $(whoami)后重启，设置smb密码：sudo smbpasswd -a $(whoami)
