...
`
  ...
  * 6.Wine使用前缀/容器（Prefix）来隔离不同的应用程序环境。默认会创建一个64位容器，路径在~/.wine。创建新的64位容器使用：`WINEPREFIX=<你设置的路径> winecfg`，创建32位容器使用：`WINEARCH=win32 WINEPREFIX=<你设置的路径> winecfg`，创建支持中文的带上`LC_ALL=zh_CN.UTF-8`，即：`WINEPREFIX=<你设置的路径> LC_ALL=zh_CN.UTF-8 winecfg`，实际上路径都建议创建在/home/你所在的用户目录。以上创建容器操作在安装winetricks后也可以在图形界面下操作。若必要使用32位容器来运行32位非安装的程序，要在命令行运行：`WINEPREFIX=~/<你所创建的32位容器路径> wine 你要运行的32位程序`，或向32位容器安装程序（方法多种自行查看图形操作）。
