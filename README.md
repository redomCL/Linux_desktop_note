...
* 3、AMDVLK开源驱动：AMD的开源驱动项目，貌似可以更持久的维护显卡，但是只支持RX400及以上的显卡，更老的显卡没有开发相应的驱动可使用。
...
* 虚拟机暂且不提，我一共测试了三套不同年代的平台，他们分别是笔记本AMD A8-4500M，笔记本英特尔 酷睿I5-7300HQ + 1050Ti，台式AMD 锐龙5900X + 1060 + B450F，通过真机安装Ubuntu测试，其他硬件驱动似乎都已经被Ubuntu内置集成好的驱动良好的驱动起来了，部分硬件比如无线网卡安装Ubuntu后默认是没有驱动成功的，通过有线网卡或其他可以直接驱动起来的无线网卡，联网更新也可以驱动起来，大多数硬件没有问题，少数Ubuntu没有为其集成驱动的依然是有问题的，这部分硬件的解决办法要么是找硬件厂商网站查看是否有相应驱动，要么去其他网站寻找代码自行编译，还是有一定门槛的。所以我的建议还是尽可能选择Ubuntu，这样默认或者通过后续联网就能支持相对更多硬件。硬件的驱动是否被Ubuntu集成，很大程度的决定因素应该是他本身的驱动是否开源了，退一步至少他应该发布过闭源驱动，再退一步恐怕就只能是至少被人逆向出来过。此外Ubuntu和其他Linux桌面发行版一般是不内置闭源驱动的，这类驱动被收纳在库里，需要安装系统后联网选择，更激进的Linux桌面发行版甚至根本就不收纳闭源形式的驱动。
...
`
  * 3.添加Wine仓库并刷新仓库（对于ubuntu/mint）：`sudo wget -NP /etc/apt/sources.list.d/ https://dl.winehq.org/wine-builds/ubuntu/dists/noble/winehq-noble.sources`，`sudo apt update`
  * 4.刷新仓库后安装Wine(稳定版winehq-stable/预发布版winehq-staging/开发版winehq-devel，各种版本自选)：`sudo apt install --install-recommends winehq-staging`
  * 5.安装完成后是不会显示图标的，也不会弹出界面，终端下手动弹出Wine配置界面，`winecfg`，一般会提示安装Windows mono，如果因为各种原因没有安装成功，可以在 https://dl.winehq.org/ 下载后使用wintricks本地安装，后续使用wintricks图形工具也可以完善环境：`sudo apt install winetricks`，一些wintricks快速完善环境的命令，如：`winetricks vcrun2022`，`winetricks allfonts corefonts cjkfonts`，`winetricks d3dx9 d3dx10`，尤其注意中文乱码：`winetricks cjkfonts`，其他字体问题尝试：`winetricks riched20 riched30`，对于具体需要哪些环境，对于Windows已经很了解，此处省略。
  * 6.Wine使用前缀/容器（Prefix）来隔离不同的应用程序环境。默认会创建一个64位容器，路径在~/.wine。创建新的64位容器使用：`WINEPREFIX=<你设置的路径> winecfg`，创建32位容器使用：`WINEARCH=win32 WINEPREFIX=<你设置的路径> winecfg`，创建支持中文的带上`LC_ALL=zh_CN.UTF-8`，即：`WINEPREFIX=<你设置的路径> LC_ALL=zh_CN.UTF-8 winecfg`，实际上路径都建议创建在/home/你所在的用户目录。以上创建容器操作都是在终端中操作，也可安装winetricks使用它在图形界面下操作（每次打开会有兼容性问题警告，并非出错）。若必要使用32位容器来运行32位非安装的程序，要在命令行运行：`WINEPREFIX=~/<你所创建的32位容器路径> wine 你要运行的32位程序`，或向32位容器安装程序（方法多种自行查看图形操作）。
  * 7.最后的可逆：卸载Wine：`sudo apt remove winehq-staging`，移除Wine仓库：`sudo rm /etc/apt/sources.list.d/winehq.list`，删除 GPG 密钥：`sudo rm /etc/apt/keyrings/winehq-archive.key`。
  * 8.随着Wine和Linux更新，当前笔记可能过时出错，最终部署步骤应以官网为准：https://www.winehq.org/
...
* 安卓：目前用waydroid，waydroid不是虚拟化，与docker一样，也是一种容器化。具体以官网部署教程为准：https://docs.waydro.id/usage/install-on-desktops ，这里主要讲一下初始化过程中下载lineage太慢，用浏览器下载rom后本地安装
  ...
  * 3.在weston中启动终端，`waydroid show-full-ui`则可以启动waydroid lineage
  * 4.可以通过一下命令指定分辨率让weston全屏启动，如768P全屏启动：`weston --width=1366 --height=768 --fullscreen`
  * 5.对于x86架构下对arm的转译探索中...
